...
class FlowerBloomApp {
    constructor() {
        ...
        // Particles
        this.particles = [];
        this.messageUnlocked = false;
        this.messageReveal = 0;
    ...
    wrapText(text, maxWidth) {
        ...
        for (const word of words) {
            ...
            if (line && this.ctx.measureText(nextLine).width > maxWidth) {
                lines.push(line);
                line = word;
            } else {
                line = nextLine;
    ...
    drawLoveNote(anchorX, anchorY) {
        if (this.messageReveal <= 0.01) return;

        const ctx = this.ctx;
        const cw = this.canvas.width;
        const ch = this.canvas.height;
        const screenX = cw - anchorX;
        const screenY = anchorY;
        const alpha = this.messageReveal;
        const sidePadding = 20;
        const cardWidth = Math.min(430, Math.max(220, cw * 0.3));
        const titleSize = Math.max(20, Math.min(26, cw * 0.022));
        const bodySize = Math.max(13, Math.min(17, cw * 0.0125));
        const titleFont = `700 ${titleSize}px Inter, sans-serif`;
        const bodyFont = `400 ${bodySize}px Inter, sans-serif`;
        const bodyLineHeight = bodySize + 7;
        const titleLineHeight = titleSize + 8;
        const cardPaddingX = 22;
        const cardPaddingY = 18;
        const gapAfterTitle = 14;
        const gapAfterBody = 10;
        ...
        const blocks = [
            {
                ...
                gapAfter: gapAfterTitle,
            },
            {
                text: 'If my soul ever wrote a love story, every page would begin and end with you.',
                font: bodyFont,
                color: 'rgba(255, 236, 244, 0.96)',
                lineHeight: bodyLineHeight,
                gapAfter: gapAfterBody,
            },
            {
                text: "And if life gave me only two days left, I'd spend the first finding my way to you... and the second never letting you leave my arms.",
                font: bodyFont,
                color: 'rgba(255, 236, 244, 0.96)',
                lineHeight: bodyLineHeight,
                gapAfter: gapAfterBody,
            ...
            {
                text: 'In every lifetime, in every version of me, my heart will always choose you.',
                font: bodyFont,
                color: 'rgba(255, 236, 244, 0.96)',
                lineHeight: bodyLineHeight,
                gapAfter: gapAfterBody,
            ...
            {
                text: 'I love you endlessly, my Meow. 🤍',
                font: `600 ${bodySize + 1}px Inter, sans-serif`,
                color: '#fff1f7',
        ...
        const layout = [];
        let cardHeight = cardPaddingY * 2;
        for (let i = 0; i < blocks.length; i++) {
            const block = blocks[i];
            ctx.font = block.font;
            const lines = this.wrapText(block.text, cardWidth - cardPaddingX * 2);
            layout.push({ ...block, lines });
            cardHeight += lines.length * block.lineHeight;
            if (i < blocks.length - 1) {
                cardHeight += block.gapAfter;
            }
        }

        const rightSpace = cw - screenX - 36 - sidePadding;
        const leftSpace = screenX - 36 - sidePadding;
        const preferRight = rightSpace >= 220 || rightSpace >= leftSpace;

        let cardX = preferRight ? screenX + 36 : screenX - 36 - cardWidth;
        cardX = Math.max(sidePadding, Math.min(cw - sidePadding - cardWidth, cardX));

        const lift = (1 - alpha) * 14;
        const cardY = Math.max(
            sidePadding,
            Math.min(ch - sidePadding - cardHeight, screenY - cardHeight * 0.48 - lift)
        );
        ...
        cardGradient.addColorStop(0, 'rgba(25, 10, 24, 0.86)');
        cardGradient.addColorStop(1, 'rgba(8, 6, 16, 0.9)');
        ctx.fillStyle = cardGradient;
        ctx.strokeStyle = 'rgba(255, 170, 205, 0.35)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ...
        // Accent line
        ctx.shadowBlur = 0;
        ctx.fillStyle = 'rgba(255, 170, 205, 0.65)';
        ctx.beginPath();
        ctx.roundRect(cardX + cardPaddingX, cardY + 14, Math.min(88, cardWidth - cardPaddingX * 2), 4, 2);
        ctx.fill();

        // Message text
        let textY = cardY + cardPaddingY + 6;
        ctx.textAlign = 'left';
    ...
    animate(timestamp) {
        ...
        if (!this.messageUnlocked && this.bloom > 0.75 && this.growth > 0.18) {
            this.messageUnlocked = true;
        }
        const targetMessageReveal = this.messageUnlocked ? 1 : 0;
        this.messageReveal += (targetMessageReveal - this.messageReveal) * 0.08 * dt;
